...
# ==== Text Splitting ====
text_splitter = RecursiveCharacterTextSplitter(chunk_size=12288, chunk_overlap=256)
...
test_random_n_models = True    # Do not query random N models by default
num_random_models = 2          # Number of random models to pick
...
synthesis_model = "qwen2.5:14b-instruct-fp16"  # Synthesis model
fact_check_model = "bespoke-minicheck:7b-fp16"  # Fact-checking model
check_responses = True  # Toggle fact-checking
...
print(f"=== List of Available Models ({total_models}) ===")
...
def synthesise_answers(prompt, check_responses=False):
    ...
    final_llm = ChatOllama(model=synthesis_model)  # Use the specified synthesis model
    
    synthesis_input = [
        {"role": "user", "content": f"Synthesise the following answers:\n\n{synthesis_content}"}
    ]
    
    final_synthesis = final_llm.invoke(synthesis_input)
    print(f"\nFinal Synthesis:\n{final_synthesis}")

    # Fact-checking step
    if check_responses:
        print("\n--- Fact-checking synthesis ---")
        fact_check_prompt = f"Synthesised Document:\n{synthesis_content}\nClaim: {final_synthesis}"
        fact_check_input = [
